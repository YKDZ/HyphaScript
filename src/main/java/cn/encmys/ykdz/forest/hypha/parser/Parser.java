...
import cn.encmys.ykdz.forest.hypha.function.ScriptFunction;
import cn.encmys.ykdz.forest.hypha.node.*;
...
public class Parser {
    ...
    private ASTNode parseUnpackStatement() {
        ...
        if (match(Token.Type.LEFT_BRAKETS)) {
            ...
            while (!match(Token.Type.RIGHT_BRAKETS)) {
                if (check(Token.Type.IDENTIFIER)) {
                    ...
                    throw new IllegalArgumentException("Expected identifier in array unpack pattern");
                }
            ...
            while (!match(Token.Type.RIGHT_BRACE)) {
                if (check(Token.Type.IDENTIFIER)) {
                    ...
                    throw new IllegalArgumentException("Expected identifier in object unpack pattern");
                }
                if (!check(Token.Type.RIGHT_BRACE)) consume(Token.Type.COMMA);
            }
        } else {
            throw new IllegalArgumentException("Expected '[' or '{' in unpack statement");
        }
    ...
    private ASTNode parseVariableDeclaration(boolean isInline, boolean isExported, boolean isConst) {
    ...
    private ASTNode parseExport() {
        ...
        throw new IllegalArgumentException("Wrong usage of export");
    }
    ...
    private ASTNode parseImport() {
        ...
        else throw new IllegalArgumentException("Wrong usage of import");
    }

    private ASTNode parseImportNamespace() {
    ...
    private ASTNode parseImportObjects() {
    ...
    private MemberAccess parseIdentifierChain(boolean isNew) {
        MemberAccess base = new MemberAccess(new Value(consume(Token.Type.IDENTIFIER).value()));

        while (true) {
            if (match(Token.Type.DOT)) {
                Token member = consume(Token.Type.IDENTIFIER);
                base = new MemberAccess(base, new Value(member.value()));
                ...
                // 是否是调用构造方法
                if (!isNew) base = new MemberAccess(new FunctionCall(base.getTarget(), base, arguments), new Value());
                else base = new MemberAccess(new New(base, arguments), new Value());
            } else break;
        }
    ...
    private ASTNode parseTerm() {
        ...
        while (match(Token.Type.PLUS, Token.Type.MINUS, Token.Type.POWER)) {
            Token operator = previous();
    ...
    private ASTNode parsePrimary() {
        ...
        if (check(Token.Type.LEFT_PAREN)) {
            ...
            if (match(Token.Type.LEFT_PAREN)) {
                ...
                consume(Token.Type.RIGHT_PAREN);
                return new MemberAccess(new FunctionCall(new MemberAccess(new Value()), new MemberAccess(expression, new Value()), arguments), new Value()); // 返回立即调用结果
            }

            return expression;
        }

        throw new IllegalArgumentException("Unexpected token in expression: " + tokens.get(currentIndex - 1));
    }

    private ASTNode parseTemplateString() {
    ...
    private ASTNode parseArray() {
        ...
        return new MemberAccess(new Array(nodes), new Value());
    }

    private ASTNode parseNestedObject() {
        consume(Token.Type.LEFT_BRACE);
        ...
        while (!match(Token.Type.RIGHT_BRACE)) {
            if (check(Token.Type.IDENTIFIER)) {
                ...
                throw new IllegalArgumentException("Key of nested object must be identifier or string");
            }
            if (!check(Token.Type.RIGHT_BRACE)) consume(Token.Type.COMMA);
        }
    ...
    private boolean isArrowFunction() {
        ...
        while (tempIndex < tokens.size() && tokens.get(tempIndex).type() != Token.Type.LEFT_PAREN) {
            ...
            if (tokens.get(tempIndex).type() == Token.Type.EQUALS) {
                if (tokens.get(tempIndex + 1).type() == Token.Type.GREATER) {
                    return true;
                }
            }
        }
    ...
    private Token consume(@NotNull Token.Type... expects) {
        ...
        throw new IllegalArgumentException("Unexpected token, expected " + Arrays.toString(expects) + " but given: " + tokens.get(currentIndex));
    }
