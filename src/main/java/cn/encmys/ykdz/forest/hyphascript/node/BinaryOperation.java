...
public class BinaryOperation extends ASTNode {
    ...
    public @NotNull Reference evaluate(@NotNull Context ctx) {
        ...
        if (operator == Token.Type.DOT) {
            if (!leftRef.getReferedValue().isType(Value.Type.NESTED_OBJECT, Value.Type.NULL) && !leftRef.getReferedValue().isType(Value.Type.JAVA_OBJECT))
                throw new IllegalArgumentException("Operator . can only be casted in a complex value.");
            else if (!rightRef.getReferedValue().isType(Value.Type.STRING, Value.Type.NULL))
            ...
            try {
                ...
                throw new EvaluateException(this, "Failed to access field '" + fieldName + "' on object of type " + object.getClass().getName());
            }
        }
        ...
        switch (operator) {
            case PLUS, MINUS, MUL, DIV, MOD, POWER:
                if (!leftRef.getReferedValue().isType(Value.Type.BIG_DECIMAL, Value.Type.NULL) || !rightRef.getReferedValue().isType(Value.Type.BIG_DECIMAL, Value.Type.NULL)) {
            ...
            case GREATER, LESS, GREATER_EQUAL, LESS_EQUAL, EQUAL_EQUAL, BANG_EQUAL:
                if (leftRef.getReferedValue().isType(Value.Type.BIG_DECIMAL, Value.Type.NULL) && rightRef.getReferedValue().isType(Value.Type.BIG_DECIMAL, Value.Type.NULL)) {
                    ...
                    return switch (operator) {
                        ...
                        case EQUAL_EQUAL -> new Reference(null, new Value(DecimalUtils.isEquals(lComp, rComp, ctx.getConfig().equalRoundingMode())));
                        case BANG_EQUAL -> new Reference(null, new Value(!DecimalUtils.isEquals(lComp, rComp, ctx.getConfig().equalRoundingMode())));
                        default -> throw new RuntimeException("Unsupported operator: " + operator);
                    };
                } else if (leftRef.getReferedValue().isType(Value.Type.BOOLEAN, Value.Type.NULL) && rightRef.getReferedValue().isType(Value.Type.BOOLEAN, Value.Type.NULL)) {
                    return switch (operator) {
                ...
                } else if (leftRef.getReferedValue().isType(Value.Type.STRING, Value.Type.NULL) && rightRef.getReferedValue().isType(Value.Type.STRING, Value.Type.NULL)) {
                    return switch (operator) {
            ...
            case LOGIC_AND, LOGIC_OR, BIT_AND, BIT_OR:
                if (!leftRef.getReferedValue().isType(Value.Type.BOOLEAN, Value.Type.NULL) || !rightRef.getReferedValue().isType(Value.Type.BOOLEAN, Value.Type.NULL)) {
