...
import cn.encmys.ykdz.forest.hyphascript.node.InstanceOf;
...
public class ComparisonParser implements ExpressionParser.Infix {
    ...
    public @NotNull ASTNode parse(@NotNull ParseContext ctx, @NotNull ASTNode left) {
        Token op = ctx.consume(Token.Type.LESS, Token.Type.LESS_EQUAL, Token.Type.GREATER, Token.Type.GREATER_EQUAL,
                Token.Type.INSTANCE_OF);
        ASTNode right = ctx.parseExpression(precedence());

        if (op.type() == Token.Type.INSTANCE_OF) {
            return new InstanceOf(left, right, left.getStartToken(), right.getEndToken());
        }
